<?php
namespace Phplib\Weiboapi;

Use \Phplib\Tools\Liblog;
Use \Phplib\Weiboapi\SaeTOAuth;
/**
 * 新浪微博操作类
 *
...
class SaeTClient
{
	...
	function upload( $status , $pic_path, $lat = NULL, $long = NULL )
	{
		...
		$log = new \Phplib\Tools\Liblog('uploadlog_virus', 'normal');

        $content = $this->curl_file_get_contents($pic_path);
    ...
    private function curl_file_get_contents($destUrl) {
	...
	function follow( $uid )
	{
		$param = array();
        $param['uid'] = $uid;
        return $this->oauth->post( 'https://api.weibo.com/2/friendships/create.json' , $param);
	}
