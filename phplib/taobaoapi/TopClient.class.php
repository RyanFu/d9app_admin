...
class TopClient{
	...
	private function getXmlData($strXml) {
		...
		if ($pos) {
			...
			$arrayCode = $this->objectToArray($xmlCode);
			return $arrayCode ;
		} else {
			return '';
		}
	...
	private function getTaobaoXmlData( ){
		...
		$logHandle = new \Phplib\Tools\Liblog('taobao_curl', 'normal');
		$logHandle->w_log(print_r($url, true));
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL,$url);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($ch, CURLOPT_TIMEOUT, 12);
		$result = curl_exec($ch);
		$logHandle = new \Phplib\Tools\Liblog('taobao_curl_result', 'normal');
