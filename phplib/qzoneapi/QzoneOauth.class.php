	 ...
	 */
	 function getAccessToken($callbackUrl, $type = 'code', $keys = array(), $state = NULL) {
		...
		if (is_array($token) && !empty($token['access_token'])) {
			...
			$logHandler = new \Snake\Libs\Base\SnakeLog(OAUTH_QZONEACCESS, 'normal');
    ...
    function getOpenId() {
		...
		$response = $this->oAuthRequest($this->openIdURL(), 'POST', $params);
 		if (strpos($response, "callback") !== false)
     	{
			...
			$logHandler = new \Snake\Libs\Base\SnakeLog(OAUTH_QZONEOPENID, 'normal');
