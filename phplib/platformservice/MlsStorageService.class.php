...
require_once(PLATFORM_SERVICE_PATH_LIB . '/MlsStorageServiceClient.class.php');
...
class MlsStorageService {
    ...
    static function StorageLog($interfaceType, $result/*array,FALSE*/, $timeCost) {
        if (NULL == $_logHandle) {
            $_logHandle = new \Phplib\Tools\Liblog('storage_perform', 'normal');
        }
        $choice = rand(0, 50);
        $ip = self::$_mlsStorageService->GetIp();
        $port = self::$_mlsStorageService->GetPort();
        if (5 == $choice) {
            if (false === $result) {
                $logContent = "[$interfaceType]\t[0]\t[$timeCost]";
                MlscacheLog('DEBUG', "[{$ip}:{$port}]\t[$interfaceType]\t[0]\t[$timeCost]");
            } else {
                $logContent = "[$interfaceType]\t[1]\t[$timeCost]";
                MlscacheLog('DEBUG', "[{$ip}:{$port}]\t[$interfaceType]\t[1]\t[$timeCost]");
            }
            $_logHandle->w_log($logContent);
        }

		$timeSpent = round($timeCost * 1000);
        if ($timeSpent > 1000 && !empty($_SERVER['REQUEST_URI'])) {
            $_overTimeLogHandle = new \Phplib\Tools\Liblog('storage_overtime', 'normal');
            $error = '';
            if (FALSE === $result) {
                $error = 'error';
            }
            $overtimeLogContent = "[{$ip}:{$port}]\t[{$interfaceType}]\t[{$timeSpent}]\t[{$_SERVER['REQUEST_URI']}]\t[{$error}]";
            $_overTimeLogHandle->w_log($overtimeLogContent);
        }
    }
    ...
    public static function BatchSql($sqls) {
        $mlsStorage = self::GetMlsStorage();
        $start = microtime(true);
        $ret = $mlsStorage->BatchSqlWithChoice($sqls, true);
        self::StorageLog('BatchSql', $ret, microtime(true)-$start);
        return $ret;
    }
    
    public static function ReadFromMaster($sql, $hashKey = "") {
        $mlsStorage = self::GetMlsStorage();
        $start = microtime(true);
        $ret = $mlsStorage->ReadFromMasterWithChoice($sql, true, $hashKey);
        self::StorageLog('ReadFromMaster', $ret, microtime(true)-$start);
        return $ret;
    }
