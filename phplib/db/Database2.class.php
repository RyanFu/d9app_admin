...
class Database {
	...
	public function read($sql, $params = array(), $from_master = FALSE, $hash_key = NULL) {
		...
		$this->proxy && $sql = $from_master ? "/*master*/" . $sql : $sql;
	...
	protected $last_sth; //@TODO: sort of an ugly hack for retrieving affected_rows
	protected $db_error_log; 
	protected $db_conf = array(); //the selected database conf
	private $proxy = FALSE;
	...
	protected function __construct($database) {
		...
		in_array($database, array("walrus", "whale", "dolphin")) && $this->proxy = TRUE;
	 ...
	 */
	protected function prepare($sql, $params, $type) {
	...
	public function callRegisteredShutdown() {
		$log = new \Phplib\Tools\Liblog('register_shutdown', 'normal');
		$log->w_log(print_r(array('sql' => $this->sql, 'param' => $this->param), true));	
	}
}
